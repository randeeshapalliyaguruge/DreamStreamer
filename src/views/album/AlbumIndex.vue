      ...
      <div class="flex bg-gray-100 py-24 sm:py-32">
        <div class="mx-auto max-w-7xl px-6 text-center lg:px-8">
          ...
          <ul role="list" class="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
            <li v-for="person in people" :key="person.id" class="rounded-2xl bg-gray-200 px-8 py-10">
              <RouterLink :to="`/albums/${person.id}`">
                
              <img class="mx-auto h-48 w-48 rounded-full md:h-56 md:w-56" :src="person.imageUrl" alt="" />
              <h3 class="mt-6 text-base font-semibold leading-7 tracking-tight text-black">{{ person.name }}</h3>
              <p class="text-sm leading-6 text-gray-600">{{ person.role }}</p>
              <ul role="list" class="mt-6 flex justify-center gap-x-6">
                <li>
                  <a :href="person.xUrl" class="text-gray-600 hover:text-gray-500">
                ...
                <li>
                  <a :href="person.linkedinUrl" class="text-gray-600 hover:text-gray-500">
